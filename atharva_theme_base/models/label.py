...
class ProductLabel(models.Model):
    ...
    def _compute_label_preview(self):
        for rec in self:
            rec.label_preview = f'<div class="ribbon-style-{rec.label_option}" t-att-id="{rec.label_option}"><span style="background-color:{rec.label_color}; color:{rec.label_text_color}">{rec.name if rec.name else "New"}</span></div>'

            if rec.label_option == 'option_5':
                rec.label_preview = f'<div class="ribbon-style-{rec.label_option}" t-att-id="{rec.label_option}"><span style="background-color:{rec.label_color} ; color:{rec.label_text_color}"><i class="fa fa-tag"/>{rec.name if rec.name else "New"}</span></div>'
